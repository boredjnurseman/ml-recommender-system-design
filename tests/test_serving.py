...
baseline_policy = load_policy(ROOT / "config/policy-v1.yaml")
experimental_policy = load_policy(ROOT / "config/policy-v2.yaml")


def test_reranker_disabled_falls_back_to_baseline():
    baseline = serve_baseline(snapshot1, "UK")
    reranked = serve_recommendations(
        snapshot1,
        "UK",
        baseline_policy,
        experimental_policy,
